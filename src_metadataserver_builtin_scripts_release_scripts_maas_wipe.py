...
def list_disks():
    ...
    """Get disks from fdisk to filter out virtual disks"""
    output = subprocess.check_output("fdisk -l | grep '^Disk /' | awk '{print $2}' | sed 's/://' ", shell=True, text=True)
    fdisks = output.strip().split("\n")
    fdisks = [disk.replace('/dev/', '').encode() for disk in fdisks]

    """Return a subset of disks and fdisks"""
    return list(set(fdisks) & set(disks))
